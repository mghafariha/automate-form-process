...
class ComputedQuery extends React.Component {
    ...
    loadPosts() {
        ...
        let { containFields,containMasterFields } = this.props;
        ...
        let masterFieldStr=new RegExp(Object.keys(this.props.reduxMasterContainFields).map((key)=>("{{m."+key+"}}")).join("|"),"gi")
        query= query.replace(masterFieldStr,matched=>(this.props.reduxMasterContainFields[matched.substring(4,matched.length-2)]));
    ...
    render(){
        let { InternalName, value, defaultValue } = this.props.field;
...
const mapStateToProps = (state, props) =>
 ({
    ...
    reduxContainFields: props.containFields.reduce(
        (acc, internalName) => Object.assign(acc, {[internalName]: state.fields[props.masterdetailFieldName].rows[props.rowId][props.internalName].value}),
        {}
       ),
       reduxMasterContainFields: props.masterContainFields.reduce(
